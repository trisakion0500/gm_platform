...
--        project_id 있음  : 호출자가 해당 프로젝트에 실제 caller_role_code(20/30)로 배정된 경우만 반환
--        project_id 없음  : company/user 테이블 로그(특정 프로젝트와 무관) — 기존대로 회사 스코핑
-- --------------------------------- --
    ...
    WHERE (
            i_caller_role_code = 10
            OR (la.`project_id` IS NOT NULL AND FN_GET_PROJECT_ROLE_CODE(i_caller_user_id, la.`project_id`) = i_caller_role_code)
