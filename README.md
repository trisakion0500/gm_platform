...
이 프로젝트의 핵심은 **"API를 데이터로 정의하면 화면이 자동으로 생성된다"**는 것이다. 아래 두 GIF는 같은 API(유저정보 — 연령/가입일시 파라미터)를 관리자가 설정하는 과정과, 그 설정대로 화면이 자동 생성되어 사용자가 실제로 호출하는 과정을 나란히 보여준다.

**1. API 설정 — 공통 코드(연령/성별)를 등록하고, 그 코드를 참조해 API 요청/응답 파라미터를 데이터로 정의**

![API 설정 데모](docs/screenshots/demo_api_setup.gif)

**2. API 호출 — 위 설정 그대로 입력 폼과 결과 화면이 자동 생성됨**

![API 호출 데모](docs/screenshots/demo_api_call.gif)

전체 화면 캡처는 [`docs/screenshots/`](docs/screenshots/) 폴더에서 확인할 수 있다.
