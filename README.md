...
- ✅ Frontend 구현 — [17_FRONTEND_BUILD_PLAN.md](docs/17_FRONTEND_BUILD_PLAN.md) 기준 Stage 0~7 전체 완료
...
- ✅ MCP 서버(mcp_server_pc 확장) — mcp_server_dev의 tool(실행/승인 + RAG 검색 3종 포함 13개)에 회사·프로젝트·사용자·역할배정 admin CRUD 20개를 더한 상위 호환 stdio MCP 서버. 계정 role_code별 노출 tool 게이팅(SUPER_ADMIN 33개/DEVELOPER 23개/APPROVER 16개/OPERATOR 12개, RAG_ENABLED 기준) 및 Claude Code·Claude Desktop 앱 양쪽 등록·연결까지 검증 완료 ([20_MCP_SERVER_PC.md](docs/20_MCP_SERVER_PC.md))
