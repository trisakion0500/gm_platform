...
본인이 실제 활성 `user_role`을 가진 프로젝트만 조회 가능 (같은 회사 소속이어도 role 미배정 프로젝트는 제외 — company 단위가 아닌 project 단위 스코핑)
...
위 "일반 사용자" 규칙과 동일 — 본인이 실제 활성 `user_role`을 가진(승인 대상) 프로젝트의 모든 실행 이력 조회 가능
