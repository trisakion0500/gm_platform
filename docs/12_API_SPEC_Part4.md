...
SUPER_ADMIN 제외, 모든 사용자는 대상 프로젝트에 실제 활성 `user_role`이 있는 경우에만 Code Group / Code Item을 조회할 수 있다.

- 단건 조회(2.3, 3.3) : 미보유 시 존재 자체를 숨기고 기존 오류코드(각각 31004, 31005)로 응답한다.
- 목록/연동 조회(2.2, 3.2, 4.1, 4.2) : project_id는 호출자가 이미 아는 값이라 숨길 필요가 없어 20001을 반환한다.
