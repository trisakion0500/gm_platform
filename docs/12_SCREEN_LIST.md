...
- **주요 기능:** 사용자 목록 조회, 회사 콤보박스(전체 회사/개별 회사) + 상태 콤보박스(전체/승인대기/정상/반려/사용중지) 필터, 페이지네이션, 상세 이동. 회사 필터는 SUPER_ADMIN·DEVELOPER 모두에게 노출(Project 목록과 동일한 패턴) — DEVELOPER는 본인 소속 회사 외를 선택해도 서버 스코핑으로 결과가 비어 보임
- **연관 API:**
