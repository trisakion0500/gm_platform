...
역할 정의 및 메뉴별 접근 권한 상세는 `14_MENU_PERMISSION.md` 참고.
...
| SCR-020 | 프로젝트 목록       | `/admin/projects`                     | O           | O         | -        | -        | DEVELOPER: 실제 DEVELOPER 역할보유 프로젝트만            |
...
- **Route:** `/signup`
- **접근:** 미인증 사용자
- **주요 기능:** 회사코드 / 프로젝트코드(선택) 직접 입력("담당자에게 문의하여 코드를 받아 입력하세요" 안내), 로그인 ID / 이름 / 이메일 / 휴대폰번호 / 부서(선택) / 직급(선택) / 비밀번호 입력, 가입 후 승인 대기 안내. 회사/프로젝트 목록 드롭다운이 아니라 코드 텍스트 입력인 이유는 `GET /companies`/`GET /projects`가 인증 필수라 로그인 전 화면에서 못 쓰기 때문(`17_FRONTEND_BUILD_PLAN.md` Stage 6 참고). 코드 검증은 입력 중이 아닌 제출 시점에만 수행.
- **연관 API:**
...
- **접근:** SUPER_ADMIN, DEVELOPER (DEVELOPER: 본인이 실제 DEVELOPER 역할로 배정된 프로젝트만)
...
- **접근:** SUPER_ADMIN (전체 수정), DEVELOPER (조회 + API Base URL/API 키만 수정, 본인이 실제 DEVELOPER 역할로 배정된 프로젝트에 한함)
