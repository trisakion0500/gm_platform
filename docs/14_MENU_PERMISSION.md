...
| 메뉴               | SUPER_ADMIN | DEVELOPER  | APPROVER   | OPERATOR     |
| ------------------ | :---------: | :--------: | :--------: | :----------: |
| 실행 이력 목록 조회 | O (전체)   | O (역할보유 프로젝트만) | O (역할보유 프로젝트만) | O (역할보유 프로젝트 내 본인만) |
| 실행 이력 상세 조회 | O          | O          | O          | O            |
| 승인 대기 목록     | O           | O          | O          | -            |
| 실행 승인          | O           | O          | O          | -            |
| 실행 반려          | O           | O          | O          | -            |
...
> SUPER_ADMIN 외에는 "본인 소속 회사"가 아니라 "본인이 실제 활성 `user_role`을 가진 프로젝트"만 반환된다(company 단위가 아닌 project 단위 스코핑, 감사 로그와 달리 role_code 종류는 가리지 않고 배정 여부만 검사). OPERATOR는 여기에 더해 본인이 요청한 건으로 한 번 더 좁혀진다.
