...
```text
30000~30999
32000~32999 (32002 제외 — 32002는 409 Conflict)
```
...
낙관적 동시성 충돌 — 조회 이후 다른 요청이 먼저 데이터를 변경해 현재 요청을 반영할 수 없음
...
```text
프로젝트 연결 정보(api_base_url) 변경과 API 키 발급이 동시에 처리되어 반영 실패
```
...
## 6.2 CSP (Content-Security-Policy) 정책
...
| 31005 | 코드 아이템 없음        |
| 31006 | API 없음                |
| 31007 | API Request 파라미터 없음 |
| 31008 | API Response 파라미터 없음 |
| 31009 | API 실행 이력 없음      |
| 31010 | 감사 로그 없음          |

---

## 32000 — Validation (중복/충돌)

| 코드  | 설명                          |
| ----- | ----------------------------- |
| 32001 | 중복 데이터                   |
| 32002 | 동시 수정 충돌 (낙관적 동시성) |
