...
- **SQL Injection 원천 차단**: 애플리케이션 레이어가 SQL 문자열을 직접 조립하지 않으므로, 문자열 결합으로 SQL을 만드는 취약점 자체가 구조적으로 존재할 수 없다.
- **권한 검증의 마지막 방어선을 DB에 둠**: 이 프로젝트는 "호출자가 특정 프로젝트/회사에 실제 권한이 있는가"를 서비스 레이어(TypeScript)에서 먼저 판단하지만, SP 내부에서도 호출자의 role_code·배정 여부를 DB에서 직접 재조회해 다시 검증한다(`FN_HAS_PROJECT_ROLE`/`FN_IS_SUPER_ADMIN` 등). 서비스 레이어에 버그가 있거나 다른 경로(배치, 내부 스크립트, 향후 추가될 클라이언트 등)로 SP가 직접 호출되어도 DB가 마지막에 한 번 더 걸러낸다 — SP가 앱이 전달한 role_code 값을 그대로 신뢰하지 않는 이유이기도 하다.
- **동시성 로직을 데이터에 가장 가까운 지점에서 원자적으로 처리**: 조건부 UPDATE + `ROW_COUNT()` 체크, `GET_LOCK` 기반 advisory lock 등 레이스 컨디션 방지 로직이 SP 내부에 있으면 트랜잭션 경계와 검증·쓰기 사이의 간극(TOCTOU)이 애플리케이션 코드보다 훨씬 좁아진다. 여러 단계로 나뉜 쓰기를 앱 레이어(ORM이든 직접 조립한 SQL이든)에서 별도의 DB 왕복으로 처리하면, 그 단계들을 하나의 트랜잭션으로 묶는 책임이 매번 개발자에게 남고 빠뜨리기도 쉽다 — 이 프로젝트도 실제로 이 문제를 겪었다: 여러 SP에서 "권한 검증"과 "쓰기"를 별도 라운드트립 두 번으로 나눠 호출했더니, 그 사이 시간차를 노려 검증 통과 직후 권한이 회수돼도 쓰기가 그대로 통과되는 TOCTOU 레이스가 실제로 발생했다(`assertProjectRole` 관련, 해결됨). 검증과 쓰기를 SP 하나(단일 트랜잭션)로 합치고 나서야 그 창이 완전히 닫혔다.
- **여러 클라이언트가 동일한 규칙을 공유**: `server/`(REST API)뿐 아니라 배치/내부 스크립트도 같은 SP를 호출하므로, 검증·정합성 규칙이 클라이언트마다 중복 구현되지 않고 SP 한 곳에만 존재한다.
