...
- 관리 화면(등록·수정·상세)에 들어가 있는 동안에는 실수로 다른 프로젝트를 보고 있다고 착각하지 않도록 헤더의 회사/프로젝트 선택이 잠깁니다. 목록 화면과, 헤더 선택으로 검색 범위를 직접 좁히는 감사로그 검색 화면(7.2)에서는 자유롭게 바꿀 수 있습니다.
...
| 프로젝트 목록/상세 조회 | O (전체) | O (실제 DEVELOPER 배정 프로젝트만) | - | - |
...
| 실행 승인/반려 | O | O | O | - |
| 실행 이력 조회 | O (전체) | O (역할보유 프로젝트만) | O (역할보유 프로젝트만) | O (역할보유 프로젝트 내 본인 요청건만) |
| 문서·API 검색 | O | O | O | O |
| 감사 로그 조회 | O (전체) | O (프로젝트 로그: 실제 배정만 / company·user 로그: 자사만) | O (동일) | - |
| 감사 로그 검색 | O (동일) | O (동일) | O (동일) | - |

※ 문서·API·감사로그 검색은 GM Platform 운영 환경 설정에 따라 메뉴 자체가 보이지 않을 수 있습니다.
...
키워드가 정확히 기억나지 않아도 자연어로 질문하듯 입력하면 관련 문서나 API 정의를 찾아주는 검색 기능입니다. 사이드바 "문서 검색"/"API 검색" 메뉴에서 사용합니다(운영 환경 설정에 따라 메뉴 자체가 보이지 않을 수 있습니다 — 그 경우 담당자에게 문의하세요).

**문서 검색**: 이 문서를 포함한 GM Platform 설계 문서 전반을 대상으로 검색합니다. 프로젝트 구분 없이 동작합니다.

![문서 검색](screenshots/31_doc_search.png)
*그림 17. 문서 검색*

**API 검색**: 5.4에서 등록한 API 정의(이름·Request·Response)를 대상으로 검색합니다. **헤더에서 선택된 프로젝트** 안에서만 검색되며, 그 프로젝트에서 실행 가능한 `api_stage`의 API만 결과에 나타납니다(8.3의 실행 가능 역할 기준과 동일).

![API 검색](screenshots/32_api_search.png)
*그림 18. API 검색*

두 화면 모두 검색 결과는 정확한 키워드 일치가 아니라 의미가 비슷한 순으로 정렬되므로, 원하는 결과가 상위에 없다면 질문을 다르게 표현해 다시 검색해보세요.
...
관리 메뉴의 "감사로그 검색"에서 변경 내용을 자연어로 질의할 수 있습니다(예: "회사 상태 변경"). 각 결과 카드에는 무엇이 어떻게 바뀌었는지 요약한 문장이 표시되고, "더보기"를 누르면 전체 내용을 카드 안에서 바로 펼쳐볼 수 있어 상세 화면으로 이동하지 않아도 됩니다.

헤더에서 회사·프로젝트를 선택해두면 검색 범위가 그 회사(또는 프로젝트)로 좁혀지고, 선택하지 않으면(SUPER_ADMIN의 "전체") 조회 권한이 있는 로그 전체를 대상으로 검색합니다. 검색 결과는 7.1과 동일하게 본인의 실제 조회 권한 범위를 벗어나지 않습니다.

이 화면도 운영 환경 설정에 따라 메뉴 자체가 보이지 않을 수 있습니다.

![감사로그 검색](screenshots/33_audit_log_search.png)
*그림 19. 감사로그 검색*
...
**Q. "문서 검색"/"API 검색"/"감사로그 검색" 메뉴가 안 보여요.**
검색 기능은 GM Platform 운영 환경 설정에 따라 켜져 있지 않을 수 있습니다. 담당자에게 문의하세요.
